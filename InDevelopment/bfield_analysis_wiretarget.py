...
aveb_2kV_0kA = np.zeros([probes,directions,fsize])
aveb_2kV_4kA = np.zeros([probes,directions,fsize])
...
aveb_squid_2kV_0kA = np.zeros([probes,loops,fsize])
aveb_squid_2kV_4kA = np.zeros([probes,loops,fsize])
aveb_squid_1kV_0kA = np.zeros([probes,loops,fsize])
aveb_squid_1kV_4kA = np.zeros([probes,loops,fsize])
...
for probe in np.arange(probes):
    for loop in np.arange(loops):
        for shot in np.arange(numshots):
            ...
            aveb_squid_2kV_0kA[probe,loop,:]=aveb_squid_2kV_0kA[probe,loop,:]+np.sqrt(datasquid1**2)
            
            datasquid2=data2['mag_probe']['squid']['b'][probe,loop,shot,:]
            aveb_squid_2kV_4kA[probe,loop,:]=aveb_squid_2kV_4kA[probe,loop,:]+np.sqrt(datasquid2**2)
            
            datasquid3=data3['mag_probe']['squid']['b'][probe,loop,shot,:]
            aveb_squid_1kV_0kA[probe,loop,:]=aveb_squid_1kV_0kA[probe,loop,:]+np.sqrt(datasquid3**2)
            
            datasquid4=data4['mag_probe']['squid']['b'][probe,loop,shot,:]
            aveb_squid_1kV_4kA[probe,loop,:]=aveb_squid_1kV_4kA[probe,loop,:]+np.sqrt(datasquid4**2)
            
aveb_squid_2kV_0kA=aveb_squid_2kV_0kA/20.0
aveb_squid_2kV_4kA=aveb_squid_2kV_4kA/20.0
aveb_squid_1kV_0kA=aveb_squid_1kV_0kA/20.0
aveb_squid_1kV_4kA=aveb_squid_1kV_4kA/20.0
