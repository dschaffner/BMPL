...
stuffing = 2.0
mean_current = np.zeros([94])
time_s = np.array(time_s)

for shot in np.arange(numshots):
    if velsflags[shot] == 1: continue
    data1=data['discharge']['dis_I'][shot]*10.0
    ...
    start_time_index = iff.tindex_min(analysis_start_time,timeB_us)
    end_time_index = iff.tindex_min(analysis_end_time,timeB_us)
    mean_current[shot] = np.max(data1[start_time_index:end_time_index])
...
plt.scatter(mean_current,pickdeltas)
plt.title('Mean Current 25 to 30us')
fig1corr = np.corrcoef(mean_current,pickdeltas)
print(fig1corr)
