...
for shot in np.arange(numshots):
    ...
    start_time_index = iff.tindex_min(analysis_start_time,timeB_us)
    end_time_index = iff.tindex_min(analysis_end_time,timeB_us)
    inj_hel[shot] = stuffing*np.trapz(data1[start_time_index:end_time_index],time_s[start_time_index:end_time_index])
...
plt.scatter(inj_hel,pickdeltas)
plt.title('Injected Helicity 10 to 50us')
fig1corr = np.corrcoef(inj_hel,pickdeltas)
print(fig1corr)
