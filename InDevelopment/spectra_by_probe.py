...
for shot in np.arange(numshots):
    for probe_index, probe in enumerate(probelist):
        for direction_index, direction in enumerate(direction_list):
            ...
            spec_frombdot_sm[shot,probe_index,direction_index,:]=iter_smooth(pwr/(f*f))
            avebspec_frombdot[probe_index,direction_index,:]=avebspec_frombdot[probe_index,direction_index]+(pwr/(f*f))
            data1=data['mag_probe']['positions'][probe][direction]['b'][shot,:]
            f,f0,comp1,pwr,mag1,phase1,cos_phase1,interval=spec.spectrum_wwind(data1[start_time_index:end_time_index],time_s[start_time_index:end_time_index],window='hanning')
            avebspec_direct[probe_index,direction_index,:]=avebspec_direct[probe_index,direction_index]+(pwr)
