...
datafilename= 'Dataset_06262024_2kV_1p5stuff_wire_nocurrent_30shots_test.h5'#without disk
...
fsize=int((data1['mag_probe']['r']['bdot'][0,start_time_index:end_time_index].shape[0])/2)+1
avebspec_frombdot = np.zeros([probes,directions,fsize])
avebspec_frombdot = np.zeros([probes,directions,fsize])
spec_frombdot = np.zeros([probes,directions,numshots,fsize])
spec_frombdot = np.zeros([probes,directions,numshots,fsize])
...
for probe in np.arange(probes):
    for loop in np.arange():
        for shot in np.arange(numshots):
            datab=data1['mag_probe']['r']['bdot'][probe,shot,:]
            f,f0,comp1,pwr,mag1,phase1,cos_phase1,interval=spec.spectrum_wwind(datab[shot,start_time_index:end_time_index],time_s[start_time_index:end_time_index],window='hamming')
            spec_frombdot[probe,direction,shot,:]=pwr/(f*f)
            avebspec_frombdot[probe,direction,:]=avebspec_frombdot[probe,direction,:]+pwr
