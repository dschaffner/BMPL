...
Br=data['mag_probe']['r']['b'][:]
Bt=data['mag_probe']['t']['b'][:]
Bz=data['mag_probe']['z']['b'][:]
Bmod = np.sqrt(Br**2+Bt**2+Bz**2)
Bmodmean=np.mean(Bmod,2)
vf=data['vf_probe']['vf'][:]
vfmean=np.mean(vf,0)
vfmean=np.mean(vfmean,0)
current=data['discharge']['dis_I']
currentmean=np.mean(current,0)
currentmean=np.mean(currentmean,0)
voltage=data['discharge']['dis_V']
voltagemean=np.mean(voltage,0)
voltagemean=np.mean(voltagemean,0)
