...
mean_energy_density_60t160=np.zeros([6,94])
mean_energy_density_25t50=np.zeros([6,94])
peak_energy_density_fullshot=np.zeros([6,94])

for shot in np.arange(numshots):
    if velsflags[shot] == 1: continue
    for probe_index, probe in enumerate(probelist):
        data_r=data['mag_probe']['positions'][probe]['r']['b'][shot,:]
        data_t=data['mag_probe']['positions'][probe]['t']['b'][shot,:]
        data_z=data['mag_probe']['positions'][probe]['z']['b'][shot,:]
        bmod = np.sqrt(data_r**2+data_t**2+data_z**2)
        bmod_T = bmod/1e4
        b_energy_density = 0.5*(bmod_T**2)/const.mu0 #Joules/m^3
        ...
        start_time_index = iff.tindex_min(analysis_start_time,timeB_us)
        end_time_index = iff.tindex_min(analysis_end_time,timeB_us)
        mean_energy_density_60t160[probe_index,shot]=np.mean(np.array(b_energy_density[start_time_index:end_time_index]))
        
        #average 25 to 50us
        analysis_start_time = 25
        analysis_end_time = 50
        start_time_index = iff.tindex_min(analysis_start_time,timeB_us)
        end_time_index = iff.tindex_min(analysis_end_time,timeB_us)
        mean_energy_density_25t50[probe_index,shot]=np.mean(np.array(b_energy_density[start_time_index:end_time_index]))

        
        peak_energy_density_fullshot[probe_index,shot]=np.max(np.array(b_energy_density))
...
plt.scatter(mean_energy_density_60t160[pickprobe,:],pickdeltas)
plt.title('Mean ED 60 to 160us')
fig1corr = np.corrcoef(mean_energy_density_60t160[pickprobe,:],pickdeltas)
print(fig1corr)
plt.plot(mean_energy_density_60t160[pickprobe,:],mean_energy_density_60t160[pickprobe,:]*fig1corr[0][1],'o',color='red')
...
plt.subplots_adjust(left=left, bottom=bottom, right=right, top=top, wspace=wspace, hspace=hspace)
ax1=plt.subplot(1,1,1)
plt.scatter(peak_energy_density_fullshot[pickprobe,:],pickdeltas)
plt.title('Peak ED Full Shot')
fig2corr = np.corrcoef(peak_energy_density_fullshot[pickprobe,:],pickdeltas)
print(fig2corr)

fig=plt.figure(num=3,figsize=(5,3.5),dpi=300,facecolor='w',edgecolor='k')
left  = 0.15  # the left side of the subplots of the figure
right = 0.94    # the right side of the subplots of the figure
bottom = 0.2  # the bottom of the subplots of the figure
top = 0.96      # the top of the subplots of the figure
...
plt.subplots_adjust(left=left, bottom=bottom, right=right, top=top, wspace=wspace, hspace=hspace)
ax1=plt.subplot(1,1,1)
plt.scatter(mean_energy_density_25t50[pickprobe,:],pickdeltas)
plt.title('Mean ED 25 to 50us')
fig3corr = np.corrcoef(mean_energy_density_25t50[pickprobe,:],pickdeltas)
print(fig3corr)
