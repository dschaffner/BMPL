...
for shot in np.arange(25):
    datload = np.loadtxt(location+filename+str(shot+1) +
...
for scope in np.arange(8):
    ...
    for shot in np.arange(25):
        for direction in np.arange(3):
            ...
            bfilt1[direction, :, shot, scope] = butter_highpass_filter(
                barray, filtcutoff, 125e6, order=3)
        bmodarray = np.sqrt(b[0, :, shot, scope]**2 +
                            b[1, :, shot, scope]**2+b[2, :, shot, scope]**2)
        bmod[:, shot, scope] = bmodarray
        bmodfilt[:, shot, scope] = butter_highpass_filter(
            bmodarray, filtcutoff, 125e6, order=3)
...
def load_picoscope(shot_number, maxrange=5, scopenum=4, time_range=[-2.0, 198.0], location='', plot=False):
    ...
    data = np.loadtxt(location+scopename+filename +
                      str(shot_number)+').txt', skiprows=2, unpack=True)
