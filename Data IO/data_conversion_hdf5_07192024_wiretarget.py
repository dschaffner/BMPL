...
datadirectory='C:\\Users\\dschaffner\\Dropbox\\Data\\BMPL\\BMX\\2024\\07192024\\'

#numskipped=len(skipshots)
#numshots=maxshot-(startshot-1)-numskipped


runfilename = 'Dataset4_wiretarget_1p5kV_1msstuff_0kAwire_density_10shots' #shots 3 to 13 skip 11
runfilename = 'Dataset4_wiretarget_1p5kV_1msstuff_2kAwire_density_10shots' #shots 14 to 24 skip 21
runfilename = 'Dataset4_wiretarget_1p5kV_1msstuff_4kAwire_density_10shots' #shots 25 to 34 no skip
runfilename = 'Dataset4_wiretarget_1p5kV_1msstuff_6kAwire_density_10shots' #shots 36 to 46 skip 42
runfilename = 'Dataset4_wiretarget_1p5kV_1msstuff_8kAwire_density_10shots' #shots 52 to 63 skip 59,60
...
5 probes:
    ...
    isat probe at opposite side at port 25 at axial center (ion collector tip away from gun)
    
    Bdots at 500mV/div on scope
    isat probe at 1V/div
"""
...
testpicoshot=pd.read_csv(datadirectory+'\pico1\\20240719-0001 ('+str(6)+').csv',header=[0],skiprows=[1])
...
for shot in np.arange(startshot,maxshot):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico=pd.read_csv(datadirectory+'\Pico1\\20240719-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
...
for shot in np.arange(startshot,maxshot):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico=pd.read_csv(datadirectory+'\Pico2\\20240719-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
...
for shot in np.arange(startshot,maxshot):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico=pd.read_csv(datadirectory+'\Pico3\\20240719-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
...
for shot in np.arange(startshot,maxshot):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico=pd.read_csv(datadirectory+'\Pico4\\20240719-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
...
for shot in np.arange(startshot,maxshot):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico=pd.read_csv(datadirectory+'\Pico5\\20240719-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
...
for shot in np.arange(startshot,maxshot):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico=pd.read_csv(datadirectory+'\Pico6\\20240719-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
    ...
    isat[savenumber,:]=np.array(pico['Channel D'],dtype=float)*200.0/100.0#Amps
    #End of loop
    savenumber+=1
##END PICO6 READIN #####
