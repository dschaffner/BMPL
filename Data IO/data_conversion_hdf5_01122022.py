...
datadirectory='C:\\Users\\dschaffner\\Dropbox\\Data\\BMPL\\BMX\\2022\\01122022\\'
maxshot=111
#shots that should be skipped
#43, 71, 72, 73, 76, 80, 82, 87
#89, 90, 91, 92, 93, 94, 100
#102, 106
skipshots=[43,71,72,73,76,80,82,87,89,90,91,92,93,94,100,102,106]
numskipped=len(skipshots)
numshots=maxshot-numskipped

runfilename = 'Dataset_01122022'
...
testpicoshot=spio.loadmat(datadirectory+'Pico1\\20220112-0001 ('+str(1)+').mat')
...
savenumber=0
for shot in np.arange(1,maxshot+1):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico1=spio.loadmat(datadirectory+'Pico1\\20220112-0001 ('+str(shot)+').mat')
...
for shot in np.arange(1,maxshot+1):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico2=spio.loadmat(datadirectory+'Pico2\\20220112-0001 ('+str(shot)+').mat')
...
for shot in np.arange(1,maxshot+1):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico3=spio.loadmat(datadirectory+'Pico3\\20220112-0001 ('+str(shot)+').mat')
...
for shot in np.arange(1,maxshot+1):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico4=spio.loadmat(datadirectory+'Pico4\\20220112-0001 ('+str(shot)+').mat')
...
for shot in np.arange(1,maxshot+1):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico5=spio.loadmat(datadirectory+'Pico5\\20220112-0001 ('+str(shot)+').mat')
