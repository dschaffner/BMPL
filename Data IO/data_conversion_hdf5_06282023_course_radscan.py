...
datadirectory='C:\\Users\\dschaffner\\Dropbox\\Data\\BMPL\\BMX\\2023\\06272023\\'
maxshot=1
startshot=53
...
runfilename = 'Dataset_06282023_course_radscan'
...
testpicoshot=pd.read_csv(datadirectory+'\pico1\\20230627-0001 ('+str(7)+').csv',header=[0],skiprows=[1])
...
num_rad_pos = 4
numshots=10
radposarray_in = np.array([0.0,1.0,2.0,3.0])#radial positions from center in inches
...
for shot in np.arange(startshot,maxshot+1):
    ...
    pico=pd.read_csv(datadirectory+'\pico1\\20230627-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
    ...
    Bdotr2[radpos,savenumber,:]=np.array(pico['Channel C'])
    Bdott2[radpos,savenumber,:]=np.array(pico['Channel D'])
...
for shot in np.arange(startshot,maxshot+1):
    ...
    #Load Picoscope file
    #pico=np.loadtxt(datadirectory+'\pico1\\20230627-0001 ('+str(shot)+').txt',skiprows = 2, unpack = True)
    pico=pd.read_csv(datadirectory+'\pico2\\20230627-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
    
    #replace infinities (from clipping) with max range values
    pico.replace("-∞",-bdotmaxrange,inplace=True)
    pico.replace("∞",bdotmaxrange,inplace=True)
    
    Bdotz2[radpos,savenumber,:]=np.array(pico['Channel A'])
    Bdotr1[radpos,savenumber,:]=np.array(pico['Channel B'])
    Bdott1[radpos,savenumber,:]=np.array(pico['Channel C'])
    Bdotz1[radpos,savenumber,:]=np.array(pico['Channel D'])
...
datadirectory='C:\\Users\\dschaffner\\Dropbox\\Data\\BMPL\\BMX\\2023\\06282023\\'
### Radial Position 1in ###
### Extract PICO 1 ###
radpos=1
savenumber=0
...
for shot in np.arange(startshot,maxshot+1):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico=pd.read_csv(datadirectory+'\pico1\\20230628-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
...
for shot in np.arange(startshot,maxshot+1):
    if shot in skipshots:
        continue
    #Load Picoscope file
    pico=pd.read_csv(datadirectory+'\pico2\\20230628-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
