...
datadirectory='C:\\Users\\dschaffner\\Dropbox\\Data\\BMPL\\BMX\\2023\\07182023\\'
maxshot=1
startshot=53
skipshots=[1000]
...
runfilename = 'Dataset_07182023_full_radscan'
...
testpicoshot=pd.read_csv(datadirectory+'\pico1\\20230718-0001 ('+str(1)+').csv',header=[0],skiprows=[1])
...
num_rad_pos = 7 #by 0.5in from center
numshots=20 #per radial position
numprobes = 5
...
vf=np.zeros([num_rad_pos,numshots,numsamples])
...
for shot in np.arange(startshot,maxshot+1):
    ...
    pico=pd.read_csv(datadirectory+'\pico1\\20230718-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
    ...
    Bdotr[radpos,0,savenumber,:]=np.array(pico['Channel C'])*term50
    Bdott[radpos,0,savenumber,:]=np.array(pico['Channel D'])*term50
...
for shot in np.arange(startshot,maxshot+1):
    ...
    #Load Picoscope file
    #pico=np.loadtxt(datadirectory+'\pico1\\20230627-0001 ('+str(shot)+').txt',skiprows = 2, unpack = True)
    pico=pd.read_csv(datadirectory+'\pico2\\20230718-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
    
    #replace infinities (from clipping) with max range values
    pico.replace("-∞",-bdotmaxrange,inplace=True)
    pico.replace("∞",bdotmaxrange,inplace=True)
    
    Bdotz[radpos,0,savenumber,:]=np.array(pico['Channel A'])*term50
    Bdotr[radpos,1,savenumber,:]=np.array(pico['Channel B'])*term50
    Bdott[radpos,1,savenumber,:]=np.array(pico['Channel C'])*term50
    Bdotz[radpos,1,savenumber,:]=np.array(pico['Channel D'])*term50
...
for shot in np.arange(startshot,maxshot+1):
    ...
    #Load Picoscope file
    #pico=np.loadtxt(datadirectory+'\pico1\\20230627-0001 ('+str(shot)+').txt',skiprows = 2, unpack = True)
    pico=pd.read_csv(datadirectory+'\pico3\\20230718-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
    
    #replace infinities (from clipping) with max range values
    pico.replace("-∞",-bdotmaxrange,inplace=True)
    pico.replace("∞",bdotmaxrange,inplace=True)
    
    Bdotr[radpos,2,savenumber,:]=np.array(pico['Channel A'])*term50
    Bdott[radpos,2,savenumber,:]=np.array(pico['Channel B'])*term50
    Bdotz[radpos,2,savenumber,:]=np.array(pico['Channel C'])*term50
    Bdotr[radpos,3,savenumber,:]=np.array(pico['Channel D'])*term50
...
for shot in np.arange(startshot,maxshot+1):
    ...
    #Load Picoscope file
    #pico=np.loadtxt(datadirectory+'\pico1\\20230627-0001 ('+str(shot)+').txt',skiprows = 2, unpack = True)
    pico=pd.read_csv(datadirectory+'\pico4\\20230718-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
    
    #replace infinities (from clipping) with max range values
    pico.replace("-∞",-bdotmaxrange,inplace=True)
    pico.replace("∞",bdotmaxrange,inplace=True)
    
    Bdott[radpos,3,savenumber,:]=np.array(pico['Channel A'])*term50
    Bdotz[radpos,3,savenumber,:]=np.array(pico['Channel B'])*term50
    Bdotr[radpos,4,savenumber,:]=np.array(pico['Channel C'])*term50
    Bdott[radpos,4,savenumber,:]=np.array(pico['Channel D'])*term50
...
for shot in np.arange(startshot,maxshot+1):
    ...
    #Load Picoscope file
    #pico=np.loadtxt(datadirectory+'\pico1\\20230627-0001 ('+str(shot)+').txt',skiprows = 2, unpack = True)
    pico=pd.read_csv(datadirectory+'\pico5\\20230718-0001 ('+str(shot)+').csv',header=[0],skiprows=[1])
    
    #replace infinities (from clipping) with max range values
    pico.replace("-∞",-bdotmaxrange,inplace=True)
    pico.replace("∞",bdotmaxrange,inplace=True)
    
    vf[radpos,savenumber,:]=np.array(pico['Channel A'])*200.0
    Bdotz[radpos,4,savenumber,:]=np.array(pico['Channel B'])*term50

    
    #filter Bdot
    Bdotz[radpos,4,savenumber,:]=butter_bandpass_filter(Bdotz[radpos,4,savenumber,:],lowcut,highcut,fs,order=9)
     
    #Compute Magnetic Field for Bdots
    Bz[radpos,4,savenumber,:]= sp.cumtrapz(Bdotz[radpos,4,savenumber,:]/tloop_area,time_s)*1e4#Gauss
    #End of loop
    savenumber+=1
##END PICO5 READIN #####
