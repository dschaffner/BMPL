...
delaytimes = np.zeros([numshots])#storage for number of delay times from cross correlation function
...
for shot in np.arange(first_shot-1,last_shot+1):
    ...
    delayshifts[shot]=np.argmax(np.abs(corr[index_at_zero_time-indexsize_of_timerange_limit:index_at_zero_time+indexsize_of_timerange_limit]))
