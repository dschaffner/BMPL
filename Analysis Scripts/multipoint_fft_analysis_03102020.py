...
datafilename='Dataset_03102020_bdot.npz'
...
fsize=int((bdot[:,start_time_index:end_time_index,:,:].shape[1])/2)+1
avebspec_frombdot = np.zeros([3,8,fsize])
#avebspec_direct = np.zeros([7,2,fsize])
#avebmagspec = np.zeros([7,fsize])

for direction in np.arange(3):
    for pos in np.arange(8):
        specsum=0
        for shot in np.arange(25):
            f,f0,comp1,pwr,mag1,phase1,cos_phase1,interval=spec.spectrum_wwind(bdot[direction,start_time_index:end_time_index,shot,pos],time_s[start_time_index:end_time_index],window='hanning')
            specsum=specsum+(pwr/(f*f))
        avebspec_frombdot[direction,pos,:]=specsum/25.0

avebspectot=np.sum(avebspec_frombdot,0)
