...
delaytimes = np.zeros([3,3,numshots])#storage for number of delay times from cross correlation function
...
for shot in np.arange(numshots):
    for direction in np.arange(3):
        d1=data['pos3']['b'][direction_list[direction]][shot,start_time_index:end_time_index]
        d2=data['pos1']['b'][direction_list[direction]][shot,start_time_index:end_time_index]
    ...
    #delay=tau[index_at_zero_time+np.argmax(np.abs(corr))]
        ...
        d1=data['pos5']['b'][direction_list[direction]][shot,start_time_index:end_time_index]
        d2=data['pos3']['b'][direction_list[direction]][shot,start_time_index:end_time_index]
        t=timeB_s[start_time_index:end_time_index]
        tau,corr = gc.get_corr(t,d1,d2,normalized=False)
        index_at_zero_time=iff.tindex_min(0,tau)
        indexsize_of_timerange_limit = int(np.round(timerange_limit/dt))
    ...
    #delay=tau[index_at_zero_time+np.argmax(np.abs(corr))]
        ...
        d1=data['pos7']['b'][direction_list[direction]][shot,start_time_index:end_time_index]
        d2=data['pos5']['b'][direction_list[direction]][shot,start_time_index:end_time_index]
        t=timeB_s[start_time_index:end_time_index]
        tau,corr = gc.get_corr(t,d1,d2,normalized=False)
        index_at_zero_time=iff.tindex_min(0,tau)
        indexsize_of_timerange_limit = int(np.round(timerange_limit/dt))
...
velocities = velocities.reshape(3*3*numshots)
...
savefilename='1msGas_0p5msStuffing_'+str(start_time)+'to'+str(end_time)+'us.png'
